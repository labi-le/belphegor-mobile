...
| Row subtitle (Subheadline) | 15 regular, `label_secondary` |
| Footnote (meta, section footer, dim text) | 13 regular, `label_secondary` |
...
- `fieldRow(label, placeholder)` — 44 dp row: label 17 leading, borderless right-aligned `EditText` 17 trailing (`textAlignment = viewEnd`), placeholder in `label_secondary`. No boxes, no floating labels.
...
- Status dot — 10 dp oval: `ios_green` running / `ios_gray` stopped.
...
- **Dialogs**: MaterialAlertDialog on themed surfaces; discovered peers as `tintedButton` list.
...
- System font, not SF Pro (license). Material ripple/switch motion, not UIKit physics.
